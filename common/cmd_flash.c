...
/* For better Flash function : by KB Kim 2010_06_17 */
int FlashCheckSector(ulong addr_first, ulong addr_last, ulong *sector_first, ulong *sector_last)
{
	flash_info_t *info;
	ulong bank;
	int rcode = 0;
	...
	{
		printf("Wrong address : 0x%x - 0x%x\n", addr_first, addr_last);
		return (-1);
	}
	...
	*sector_last  = 0;
	...
	for (bank=0, info=&flash_info[0];  (bank < CFG_MAX_FLASH_BANKS) ; ++bank, ++info)
	{
		...
		for (sect=0; sect < info->sector_count; ++sect) {
			...
			if ((addr_first >= info->start[sect]) && (addr_first <= end))
			{
				*sector_first = info->start[sect];
				printf("Start Address : 0x%x -> 0x%x\n", addr_first, *sector_first);

			}
			if ((addr_last >= info->start[sect]) && (addr_last <= end))
			{
				*sector_last = end;
				printf("End Address : 0x%x -> 0x%x\n", addr_last, *sector_last);
			}
		}
	}

	if ((*sector_first == 0) || (*sector_last ==0))
	{
		return (-1);
	}
	...
	*s_count = 0;
	...
	for (bank=0,info=&flash_info[0];
		...
		if (s_first[bank] >= 0) {
            ...
            printf("start addr=0x%x, sector boundary =0x%x - 0x%x\n",addr_first, info->start[0], b_end);
...
int
do_addr_spec(char *arg1, char *arg2, ulong *addr_first, ulong *addr_last)
{
	return (addr_spec(arg1, arg2, addr_first, addr_last));
}
...
int flash_sect_erase (ulong addr_first, ulong addr_last)
{
	...
	if (FlashCheckSector(addr_first, addr_last, &adAddrStart, &adAddrLast) < 0)
	{
		return 1;
	}
...
#else
	rcode = flash_fill_sect_ranges (adAddrStart, adAddrLast,
					s_first, s_last, &planned );
#endif
	if (planned && (rcode == 0)) {
		for (bank=0,info=&flash_info[0];
			...
			if (s_first[bank]>=0) {
				...
				debug ("Erase Flash from 0x%08lx to 0x%08lx "
					...
					bank+1);
				// printf ("Erase Bank[%d] Sector From [%d] to [%d]\n", bank, s_first[bank], s_last[bank]);
