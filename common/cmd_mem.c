...
int do_mem_check (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong	addr, value;
	int	size;
	int     rcode = 0;
	...
	/* Check for size specification.
	*/
	if ((size = cmd_get_data_size(argv[0], 5)) < 0)
		return 1;
    ...
    if (size == 4) {
        ulong word = *(ulong *)addr;
        if (word != value) {
            printf("word at 0x%08lx (0x%08lx) "
                   "!=  checked value (0x%08lx)\n",
                   addr, word, value);
            rcode = 1;
        ...
        ushort hword = *(ushort *)addr;
        if (hword != (ushort) value) {
            printf("halfword at 0x%08lx (0x%04x) "
                   "!= checked value (0x%04x)\n",
                   addr, hword, value);
            rcode = 1;
        ...
        u_char byte = *(u_char *)addr;
        if (byte != (u_char) value) {
            printf("byte at 0x%08lx (0x%02x) "
                   "!= checked value (0x%02x)\n",
                   addr, byte, value);
            rcode = 1;
