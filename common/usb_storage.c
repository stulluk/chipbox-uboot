...
unsigned int Fs_Block_Size = 512;
...
/* For Front Display during Flash Access by KB Kim 2010.12.27 */
extern int FbSendFndDisplay(char *data);
...
unsigned long usb_stor_read(int device, unsigned long blknr, unsigned long blkcnt, unsigned long *buffer)
{
	...
	ccb *srb = &usb_ccb;
	/* For Front Display during Flash Access by KB Kim 2010.12.27 */
	char frontDisplay[10];
	int  count, count10;

	/* For Front Display during Flash Access by KB Kim 2010.12.27 */
	memset(frontDisplay, 0x00, 10);
	count   = 0;
	count10 = 0;
...
retry_it:
		if(smallblks==USB_MAX_READ_BLK)
		{
			...
			if (count10 == 10)
			{
				sprintf (frontDisplay, "r%03d", count);
				FbSendFndDisplay(frontDisplay);
				count++;
				if (count >= 1000)
				{
					count -= 1000;
...
int usb_stor_get_info(struct usb_device *dev,struct us_data *ss,block_dev_desc_t *dev_desc)
{
	...
	if (
	    ...
	    (dev->descriptor.idVendor == 0x0781 &&
	     ...
	     ||
	    /* SanDisk Corporation Cruzer Crossfire -- added by sunbin:sunbin1010@163.com 20081216 */
	    (dev->descriptor.idVendor == 0x0781 &&
	     dev->descriptor.idProduct == 0x5204)
	    )
	...
	USB_STOR_PRINTF("VenderId : 0x%X, ProductId : 0x%X\n", dev->descriptor.idVendor, dev->descriptor.idProduct);

	pccb->pdata = usb_stor_buf;

	// if( (dev->descriptor.idVendor == 0x17ef && dev->descriptor.idProduct == 0x383c) || (dev->descriptor.idVendor == 0x90c && dev->descriptor.idProduct == 0x1000) )
	if( (dev->descriptor.idVendor == 0x17ef && dev->descriptor.idProduct == 0x383c))
		dev_desc->lun = 1;
	...
	if(usb_inquiry(pccb,ss)){
		ss->transport_reset(ss);
		printf("uboot-usb: inquiry failed --> reset device...\n");
		if(usb_inquiry(pccb,ss))
			return -1;
	}
	...
	memset(buffer,7,2048);
	if (dev_desc->block_read(dev_desc->dev, 0, 1, (unsigned long *) buffer) == 1){
		if( (0 == buffer[512]) && (0 == buffer[2047]) )
			Fs_Block_Size = 2048;
		else
			Fs_Block_Size = 512;
	}
	else
		printf("uboot-usb: maybe device error...\n");
