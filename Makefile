...
ifeq ($(BOARD),orion)
LIBS += drivers/orion_tuner/libtuner.a
LIBS += drivers/orion_xport/libxport.a
endif
...
all:  clean_secu_loader	$(ALL)
clean_secu_loader:
		rm -f $(TOPDIR)/secu_loader.bin

$(obj)secu_loader.rom.bin:
#	cd $(obj)sldr/build/arm_gcc && ./make.bash && cp secu_loader.rom.bin $(TOPDIR)
...
$(obj)u-boot.bin:	$(obj)u-boot 
		./sldr/make.sh
		$(OBJCOPY) ${OBJCFLAGS} -O binary $< $@
		cp ./sldr/bin/secu_loader.rom ./secu_loader.bin
		dd if=./sldr/ddr_config.bin of=secu_loader.bin bs=384 seek=10
		cat $(obj)secu_loader.bin > su-boot.bin		
		dd if=$@ of=su-boot.bin bs=1K seek=4
		@if [ -n "$(TFTPT)" ]; then cp -v $@ su-boot.bin $(TFTPT); fi
endif
...
cscope:
		echo "include/configs/$(BOARD).h" > cscope.files
		find 		include sldr \
				lib_generic board/$(BOARDDIR) cpu/$(CPU) lib_$(ARCH) \
				fs/cramfs fs/fat fs/fdos fs/jffs2 \
				net disk rtc dtt drivers drivers/sk98lin common \
				\( -name CVS -prune \) -o \( -name configs -prune \) \
				-o \( -name '*.[sSch]' -print \) >> cscope.files
		cscope -kbq
...
mrhgold_256m_32mflashmode_config \
mrhgold_256m_16mflashmode_config \
mrhgold_256m_config \
mrhgold_config \
orion_config	\
orion13_config	\
orion14_nand_config \
orion14_nand_16bit_config \
orion14_nand_256m_config \
orion14_16bit_config \
orion14_256m_config \
orion14_config \
orion14_overload_config \
csm1100_config	\
csm1200_nand_config \
csm1200_nand_16bit_config \
csm1200_nand_256m_config \
csm1200_16bit_config \
csm1200j_config \
csm1200_256m_config \
csm1200_config \
csm1200_overload_config \
csm1200_sstflash_config \
csm1201_config\
csm1201_nand_config \
csm1201_nand_16bit_config \
csm1201_nand_256m_config \
csm1201_32mflashmode_config \
csm1201_256m_32mflashmode_config \
csm1201_32mflashmode_256m_config \
csm1201_16bit_config \
csm1201_16bit_32mflashmode_config \
csm1201_256m_config \
csm1201_spi_256m_config \
csm1201j_config \
csm1201j_spi_config \
csm1201j_spi_128m_config \
csm1201j_128m_config: unconfig
	@board/orion/split_by_variant.sh $@ 
...
clean:
	find $(OBJTREE) -type f \
		...
		-o -name '*.o' -o -name '.depend' -o -name '*.a'  \) -print \
		| xargs rm -f
