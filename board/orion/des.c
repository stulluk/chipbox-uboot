...
static void DES(char Out[8], char In[8], char pSubKey[16][48], char Type);
static void SetKey(const char* Key, int len);/* 设置密钥 */
static void SetSubKey(char pSubKey[16][48], const char Key[8]);/* 设置子密钥 */
...
char Des_Go(char *Out, char *In, long datalen, const char *Key, int keylen, char Type)
{
    long i, j;
	int count = 0;
    if( !( Out && In && Key && datalen ) )
		return 0;
    datalen = datalen & 0xfffffff8;
	SetKey(Key, keylen);
	printf("\n");
	if( !Is3DES ) {   /* 1次DES */
...
void SetKey(const char* Key, int len)
{
	...
	Is3DES = len>8 ? (SetSubKey(SubKey[1], &deskey[8]), 1) : 0;
}

void DES(char Out[8], char In[8], char pSubKey[16][48], char Type)
{
...
void SetSubKey(char pSubKey[16][48], const char Key[8])
{
...
void S_func(char Out[32], const char In[48])
{
    ...
    char *ptr;
    ptr = (char *)In;
    for(i=0; i<8; ++i,ptr+=6,Out+=4) {
	j = (ptr[0]<<1) + ptr[5];
	k = (ptr[1]<<3) + (ptr[2]<<2) + (ptr[3]<<1) + ptr[4];
