...
{
	unsigned char buffer[(SECTOR_SIZE/(2048/Fs_Block_Size))];
...
get_fatent(fsdata *mydata, __u32 entry)
{
	...
	case 32:
		bufnum = entry / (FAT32BUFSIZE/(2048/Fs_Block_Size));
		offset = entry - bufnum * (FAT32BUFSIZE/(2048/Fs_Block_Size));
		break;
	case 16:
		bufnum = entry / (FAT16BUFSIZE/(2048/Fs_Block_Size));
		offset = entry - bufnum * (FAT16BUFSIZE/(2048/Fs_Block_Size));
		break;
	case 12:
		bufnum = entry / (FAT12BUFSIZE/(2048/Fs_Block_Size));
		offset = entry - bufnum * (FAT12BUFSIZE/(2048/Fs_Block_Size));
		break;
	...
	if (bufnum != mydata->fatbufnum) {
		int getsize = (FATBUFSIZE/(2048/Fs_Block_Size))/(FS_BLOCK_SIZE/(2048/Fs_Block_Size));
		__u8 *bufptr = mydata->fatbuf;
		__u32 fatlength = mydata->fatlength;
		__u32 startblock = bufnum * FATBUFBLOCKS;

		fatlength *= (SECTOR_SIZE/(2048/Fs_Block_Size));	/* We want it in bytes now */
...
get_cluster(fsdata *mydata, __u32 clustnum, __u8 *buffer, unsigned long size)
{
	...
	if (disk_read(startsect, size/(FS_BLOCK_SIZE/(2048/Fs_Block_Size)) , buffer) < 0) {
		FAT_DPRINT("Error reading data\n");
		return -1;
	}
	if(size % (FS_BLOCK_SIZE/(2048/Fs_Block_Size))) {
		__u8 tmpbuf[(FS_BLOCK_SIZE/(2048/Fs_Block_Size))];
		idx= size/(FS_BLOCK_SIZE/(2048/Fs_Block_Size));
		if (disk_read(startsect + idx, 1, tmpbuf) < 0) {
			FAT_DPRINT("Error reading data\n");
			return -1;
		}
		buffer += idx*(FS_BLOCK_SIZE/(2048/Fs_Block_Size));

		memcpy(buffer, tmpbuf, size % (FS_BLOCK_SIZE/(2048/Fs_Block_Size)));
		return 0;
	}
	     ...
	     unsigned long maxsize)
{
	...
	unsigned int bytesperclust = mydata->clust_size * (SECTOR_SIZE/(2048/Fs_Block_Size));
	     ...
	     dir_entry *retdent, char *l_name)
{
	...
	__u8	  *nextclust = cluster + mydata->clust_size * (SECTOR_SIZE/(2048/Fs_Block_Size));
	...
	if ((__u8*)slotptr >= nextclust) {
		...
		if (get_cluster(mydata, curclust, get_vfatname_block,
				mydata->clust_size * (SECTOR_SIZE/(2048/Fs_Block_Size))) != 0) {
				   ...
				   int dols)
{
	...
	for (i = 0; i < (DIRENTSPERCLUST/(2048/Fs_Block_Size)); i++) {
...
{
	__u8 block[(FS_BLOCK_SIZE/(2048/Fs_Block_Size))];
	volume_info *vistart;
	     ...
	     int dols)
{
	...
	rootdir_size = ((bs.dir_entries[1] * (int) 256 + bs.dir_entries[0])
			* sizeof (dir_entry)) / (SECTOR_SIZE/(2048/Fs_Block_Size));
	mydata->data_begin = mydata->rootdir_sect + rootdir_size
		- (mydata->clust_size * 2);
    }
    mydata->fatbufnum = -1;
    ...
    FAT_DPRINT ("Rootdir begins at sector: %d, offset: %x, size: %d\n"
		"Data begins at: %d\n",
		mydata->rootdir_sect, mydata->rootdir_sect * (SECTOR_SIZE/(2048/Fs_Block_Size)),
	...
	for (i = 0; i < (DIRENTSPERBLOCK/(2048/Fs_Block_Size)); i++) {
	    char s_name[14], l_name[256];
