...
/* This is used Used Flash Driver : By KB Kim */
...
int flash_erase (flash_info_t * info, int s_first, int s_last)
{
	...
	for (sect = s_first; sect <= s_last; sect++) {
		if (info->protect[sect] == 0) { /* not protected */
			/* For Front Display during Flash Access by KB Kim 2010.12.27 */
			sprintf (frontDisplay, "E%03d", count);
...
int write_buff (flash_info_t * info, uchar * src, ulong addr, ulong cnt)
{
	...
	//	printf("wp = 0x%x\n",wp);
		if (info->buffer_size == 1) {
			...
			{
				/* For Front Display during Flash Access by KB Kim 2010.12.27 */
				sprintf (frontDisplay, "S%03d", count);
				FbSendFndDisplay(frontDisplay);
				count++;
			}
			continue;
